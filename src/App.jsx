...
// Componente optimizado para la imagen con lazy loading
...
// Componente optimizado para la tarjeta de producto
const ProductCard = memo(({ product, onAddToCart, onSelectProduct, translateCategory }) => {
  return (
    <div className="bg-white rounded-lg md:rounded-xl shadow-lg md:shadow-xl overflow-hidden hover:shadow-2xl transition-all hover:scale-105">
      <div 
        className="aspect-square bg-gray-100 flex items-center justify-center p-2 md:p-4 cursor-pointer hover:bg-gray-200 transition-colors relative"
      ...
      <div className="p-2 md:p-4">
        <div className="text-[10px] md:text-xs text-[#ff0000] font-semibold mb-1">
        ...
        {product.description && (
          <p className="text-[10px] md:text-sm text-gray-600 mb-2 md:mb-3 line-clamp-2 hidden sm:block">
            {product.description}
          </p>
        )}
        <div className="flex items-center justify-between mb-2 md:mb-4">
          <span className="text-[9px] md:text-xs text-gray-500 truncate">
            SKU: {product.sku}
          </span>
        </div>
        <button 
...
export default function App() {
  ...
  // Calcular categorías con useEffect para optimizar
  useEffect(() => {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      const matchesSearch = product.product_name?.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
  ...
  }, [products, debouncedSearchTerm, selectedCategory, selectedSubcategory]);
  ...
  const updateQuantity = useCallback((id, cantidad) => {
    setCart(prevCart => {
      if (cantidad <= 0) {
        ...
        return prevCart.map(item => item.id === id ? { ...item, cantidad } : item);
      }
    });
  }, []);
  ...
  // Calcular totales del carrito con useMemo
  const cartTotals = useMemo(() => {
    return {
      totalItems: cart.length,
      totalQuantity: cart.reduce((sum, item) => sum + item.cantidad, 0)
    };
  }, [cart]);
  ...
  const generateQuotationImage = async () => {
    ...
    ctx.font = '20px Arial';
    ctx.textAlign = 'center';
    ctx.fillText(`Total de productos: ${cartTotals.totalItems}`, canvasWidth / 2, footerY);
    ctx.fillText(`Total de unidades: ${cartTotals.totalQuantity}`, canvasWidth / 2, footerY + 30);
          ...
          <div className="flex justify-between items-center gap-2">
            <div className="flex items-center gap-2 md:gap-4 flex-1 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs md:text-base text-white/90 hidden sm:block">Catálogo de Productos - Eash</p>
              </div>
            </div>
            <button 
